...
/* funzioni relative alla gestione delle richieste */


function leggiRichieste($cid)
{
	$richieste = array();
	...
	$sql= "SELECT sendingUsername, receivingUsername FROM richieste;";                                                 //Verificare effettivamente il DB
	$res=$cid->query($sql);
	if ($res==null)
	{
		$risultato["status"]="ko";
...
function stampaRichieste($richieste)
{
	echo "<div class=\"table-responsive\">";
	echo "<table class=\"table text-center\">";

	foreach ($richieste AS $sendingUsername)           
	{
		echo "<tr><td>$sendingUsername</td> 
		      <td><a href=\"../backend/updateS.php?id=$sendingUsername\"><span class=\"glyphicon glyphicon-pencil\"></span></a></td>
		      <td><a href=\"../backend/deleteS.php?id=$sendingUsername\"><span class=\"glyphicon glyphicon-remove\"></span></a></td></tr>";
...
function scriviRichiesta($nickname)
{	
	$risultato = array("status"=>"ok","msg"=>"", "contenuto"=>"");
	...
	{	
		$richieste=$res["contenuto"];
		
		if (isset($richieste[$sigla]))
		{
			$errore = true;
			$msg .= "La sigla $sigla specifica &egrave; gi&agrave; usata per $richieste[$sigla]<br/>";
		}
	}
	if (!$errore)
	{
	   $sql= "INSERT INTO richieste VALUES('$sigla','$nome');";
	   $res=$cid->query($sql);
	   if ($res==1)
...
function stampaPartite($partite,$squadre)
{
	...
	if (count($partite)>0)
	{
		foreach ($partite AS $partita)
		{
			...
			echo "<td>$partita[goalospite]</td>";
			echo "<td><a href=\"../backend/updateP.php?casa=$casa&ospite=$ospite\"><span class=\"glyphicon glyphicon-pencil\"></span></a></td>";
			echo "<td><a href=\"../backend/deleteP.php?casa=$casa&ospite=$ospite\"><span class=\"glyphicon glyphicon-remove\"></span></a></td></tr>";
